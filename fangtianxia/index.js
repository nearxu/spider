...
// 在售楼盘b81http://esf.cd.fang.com/house/i3";
// 在售楼盘b81
var itemUrl = "b81";
...
let DomCount = function() {
  return {
    starCount: function(el, $) {
      let orangeStar = $(el)
        .find(".nlc_details .house_value  .star_group li")
        .filter(".orange-star").length;
      let halfStar = $(el)
        .find(".nlc_details .house_value  .star_group li")
        .filter(".half-star").length;
      return orangeStar * 1 + halfStar * 0.5;
    },
    //获取楼盘类型， 改善型 ，刚需...
    getType: function(el, $) {
      let listString = $(el)
        .find(".nlc_details .fangyuan a")
        .text();
    ...
    if (response.headers["content-encoding"].indexOf("gzip") != -1) {
      zlib.gunzip(body, function(err, dezipped) {
        ...
        li.each(function(index, el) {
          let address = $(el)
            .find(".nlc_details .relative_message .address a")
            .attr("title");
          let obj = {
            name:
              $(el)
                .find(".nlc_details .house_value  .nlcd_name a")
                .text()
                .trim() || "", // 楼盘名称
            ftxhref:
              $(el)
            ...
            phoneNumber:
              $(el)
                .find(".nlc_details .relative_message .tel p")
                .text() || "", // 楼盘电话
            imageUrl:
              $(el)
                .find(".nlc_img a")
